...
import { Convert, type Roadmap } from "../../lib/transformacion"; // Importa las interfaces y la función Convert
...
constructor() {
  ...
  this.loadAndRenderRoadmap = this.loadAndRenderRoadmap.bind(this);
...
// Método para cargar y mostrar el roadmap desde un JSON y modificar los colores
loadAndRenderRoadmap(jsonUrl: string) {
  if (!jsonUrl) {
  ...
  return fetch(jsonUrl)
    .then((res) => res.json())
    .then((json: string) => {
      // Convierte el JSON a un objeto de datos utilizando la función Convert.toRoadmap
      const roadmapData: Roadmap = Convert.toRoadmap(json);

      // Modifica dinámicamente el color de los componentes del roadmap
      this.modifyRoadmapColors(roadmapData);

      // Convierte el JSON a un SVG y reemplaza el contenido del contenedor con él
...
// Método para modificar dinámicamente los colores del roadmap
modifyRoadmapColors(roadmapData: Roadmap) {
  // Obtén los controles del roadmap
  const mockupControls = roadmapData.mockup.controls.control;

  // Itera sobre los controles y modifica los colores según las etiquetas
  mockupControls.forEach((control) => {
    const label = control.properties?.text; // Obtiene la etiqueta del control

    // Modifica el color del control basado en el texto de la etiqueta
    // Modifica el color del control basado en el texto de la etiqueta
switch (label) {
case 'Herramientas':
  if (control.properties) {
    control.properties.color = 'red'; // Cambia el color a rojo
  }
  break;
case 'Sigue aprendiendo!':
  if (control.properties) {
    control.properties.color = 'blue'; // Cambia el color a azul
  }
  break;
// Agrega más casos según las etiquetas que tengas
default:
  // No hace nada o deja el color como está
  break;
}
...
onDOMLoaded() {
  ...
  if (roadmapType) {
    ...
    this.loadAndRenderRoadmap(this.jsonUrl);
...
switchRoadmap(newJsonUrl: string) {
  ...
  this.loadAndRenderRoadmap(newJsonUrl)?.then(() => {
